...
namespace CrimsonMoonArabic
{
    static class Program
    {
        ...
        {
            (@"CrimsonMoonNG\Content\Paks",   "zzz_Arabic_P.pak"),
            (@"CrimsonMoonNG\Content\Paks",   "zzz_Arabic_P.ucas"),
            (@"CrimsonMoonNG\Content\Paks",   "zzz_Arabic_P.utoc"),
            (@"CrimsonMoonNG\Content\Movies", "Intro_ar.srt"),
            (@"CrimsonMoonNG\Content\Movies", "CINE_EndGame_ar.srt"),
        };
        const string InstalledMarker = "zzz_Arabic_P.utoc";
        ...
        // ---- path helpers -----------------------------------------------------
        public static string PaksDir(string gameRoot) => Path.Combine(gameRoot, "CrimsonMoonNG", "Content", "Paks");
        public static bool IsValidGameFolder(string folder) =>
            !string.IsNullOrEmpty(folder) && File.Exists(Path.Combine(PaksDir(folder), "CrimsonMoonNG-Windows.pak"));
        ...
        public static void Install(string gameRoot, Action<string> progress)
        {
            ...
            if (!Directory.Exists(paks)) throw new Exception("لم يُعثر على مجلد اللعبة:\nCrimsonMoonNG\\Content\\Paks");
            EnsureGameClosed(Path.Combine(paks, "CrimsonMoonNG-Windows.pak"));
            ...
            using (Stream s = asm.GetManifestResourceStream("payload.zip"))
            {
                ...
                using (var z = new ZipArchive(s, ZipArchiveMode.Read))
                    foreach (var (dir, f) in ModFiles)
                    {
                        var entry = z.GetEntry(f);
                        if (entry == null) throw new Exception("ملف مفقود داخل المثبّت: " + f);
                        string target = Path.Combine(gameRoot, dir); Directory.CreateDirectory(target);
                        entry.ExtractToFile(Path.Combine(target, f), true);
                    }
            }
        ...
        public static void Uninstall(string gameRoot, Action<string> progress)
        {
            ...
            foreach (var (dir, f) in ModFiles) { string p = Path.Combine(gameRoot, dir, f); if (File.Exists(p)) File.Delete(p); }
    ...
    public class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            Font = Ui.F(11f);
            Text = "Crimson Moon Arabic Installer";
            MouseDown += DragStart;
        ...
        void OnBrowse(object sender, EventArgs e)
        {
            ...
            {
                dlg.Description = "اختر مجلد اللعبة (المجلد الذي يحتوي على CrimsonMoonNG)";
                ...
                if (dlg.ShowDialog(this) == DialogResult.OK)
                {
                    ...
                    {
                        string sub = Path.Combine(chosen, "Crimson Moon");
                        if (Program.IsValidGameFolder(sub)) chosen = sub;
                    }
                    if (Program.IsValidGameFolder(chosen)) gamePath = chosen;
                    else MessageBox.Show(this, "هذا المجلد لا يحتوي على ملفات اللعبة (CrimsonMoonNG\\Content\\Paks).",
